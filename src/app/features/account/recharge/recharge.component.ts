...
@Component({
  ...
  template: `
    <div class="page account-static-page">
      ...
      <div class="card">
        <p class="text-muted">{{ 'account.recharge.currentBalance' | translate }} <strong>{{ wallet.balance }} €</strong></p>
        ...
        <div class="form-group"><label>{{ 'account.recharge.otherAmount' | translate }}</label><input class="form-input" type="number" min="1" /></div>
      </div>
      <div class="card mt-1">
        <p>{{ 'account.recharge.cardForRecharge' | translate }}</p>
        <div class="card-row"><span>Visa •••• {{ wallet.mainCard.last4 }}</span><span class="text-muted">{{ 'account.recharge.expires' | translate }} {{ wallet.mainCard.expiryDate }}</span></div>
      </div>
      <div class="card mt-1">
        <p>{{ 'account.recharge.balanceAfter' | translate }} <strong>{{ wallet.balance + selected() }},00 €</strong></p>
      </div>
      <button class="btn btn-primary btn-block mt-2" (click)="confirm()">{{ 'account.recharge.button' | translate }}</button>
      @if (done()) {
        <div class="toast"><strong>{{ 'account.recharge.success' | translate }}</strong><br>{{ 'account.recharge.successDetail' | translate:{amount: selected() + ',00 €'} }}</div>
      }
    </div>
  `,
  styles: [`
