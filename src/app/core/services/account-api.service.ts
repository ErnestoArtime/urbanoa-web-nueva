...
export class AccountApiService {
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<void> {
    try {
      await this.api.put('/users/password', { currentPassword, newPassword });
      this.source.set('remote');
    } catch (e) {
      console.warn('[API] Cambio de contraseña usa fallback mock', this.api.errorMessage(e));
  ...
  async cancelAccount(): Promise<void> {
    try {
      ...
      console.warn('[API] Baja de cuenta usa fallback mock', this.api.errorMessage(e));
      this.source.set('mock');
    }
  }
