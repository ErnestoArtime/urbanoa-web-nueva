...
@Component({
  ...
  styles: [
    `
      :host{display:block}.detail-panel-header{display:flex;align-items:center;gap:.5rem;padding:.75rem 1rem;background:var(--color-surface);position:sticky;top:0;z-index:10}.header-title{flex:1;font-size:var(--text-md);font-weight:var(--font-extra);margin:0}.back-btn{text-decoration:none;color:var(--color-primary);font-size:1.2rem;line-height:1;padding:0;display:grid;place-items:center;width:32px;height:32px;border-radius:50%;transition:background .15s}.back-btn:hover{background:var(--color-active)}.back-desktop{display:none}.header-actions{display:flex;gap:.25rem}.icon-btn{background:none;border:none;padding:6px;cursor:pointer;color:var(--color-text-muted);border-radius:50%;display:grid;place-items:center;transition:background .15s,color .15s;width:32px;height:32px}.icon-btn:hover{background:var(--color-active);color:var(--color-text)}@media(max-width:959px){:host{display:none}}@media(min-width:960px){.back-desktop{display:grid}}
    `,
  ],
})
